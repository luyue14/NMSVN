$(document).ready(function() {
	...
	function successAfterSendEdge(time){
		
	}
	function errorAfterSendEdge(time){
	...
	function sendEdgePorts(time){
		var edge = {};
		edge.edgePorts = [];
		for(var i = 0; i < vnConfigListResult.length; i++){
			if(parseInt(vnConfigListResult[i].timeStamp / 1000) == time / 1000){
				edge.timeStamp = vnConfigListResult[i].timeStamp;
				edge.vnId = vnConfigListResult[i].vnId;
				//查找该配置方案中的边缘端口；
				for(var j = 0; j < vnConfigListResult[i].dp.length; j++){//遍历i配置方案中的所有节点；
					var tmp = {};
					tmp.nodeId = vnConfigListResult[i].dp[j].dpId;
					tmp.inPort = [];
					for(var k = 0; k < vnConfigListResult[i].dp[j].ports.length; k++){//遍历配置方案j节点的所有端口；
						if(isEdgePorts(time, vnConfigListResult[i].dp[j].dpId , vnConfigListResult[i].dp[j].ports[k])){//如果是边缘端口则添加
							tmp.inPort.push(vnConfigListResult[i].dp[j].ports[k]);
						}
					}
					if(tmp.inPort.length != 0){//如果该节点有边缘端口则添加
						edge.edgePorts.push(tmp);
					}
				}
				break;//找到对应的vn记录就可退出循环；
			}
		}
		//处理并发送边缘节点信息；
		for(var i = 0; i < edge.edgePorts.length; i++){
			for(var j = 0; j < edge.edgePorts[i].inPort.length; j++){
				edge.edgePorts[i].inPort[j] = parseInt(edge.edgePorts[i].inPort[j].substring(edge.edgePorts[i].nodeId.length+1));
			}
			edge.edgePorts[i].nodeId = parseInt(edge.edgePorts[i].nodeId.substring(9));
		}
		$.ajax(submitInfor($.toJSON(edge),url.subEdgePorts,successAfterSendEdge,errorAfterSendEdge,time));
	}
	function isEdgePorts(time,dpId,port){
		for(var i = 0; i < topoResult.link.length; i++){//遍历topo中的link查找对应端口
			if(topoResult.link[i].destination.destTp == port){//如果在dest中找到了
				if(isContainInVn(time,topoResult.link[i].source.sourceNode,topoResult.link[i].source.sourceTp)){//如果对端不在vn中，则是边缘节点；
					return false;	
				}else{
					return true;
				}
			}
			if(topoResult.link[i].source.sourceTp == port){//如果在source中找到了
				if(isContainInVn(time,topoResult.link[i].destination.destNode,topoResult.link[i].destination.destTp)){//如果对端不在vn中，则是边缘节点；
					return false;	
				}else{
					return true;
				}
		...
		return true;//在topo中没有查到该端口；
	}
	function isContainInVn(time,dpId,port){//查找对端端口是否属于虚拟网；
		for(var i = 0; i < vnConfigListResult.length; i++){
			if(parseInt(vnConfigListResult[i].timeStamp / 1000) == parseInt(time / 1000)){
				for(var j = 0; j < vnConfigListResult[i].dp.length; j++){
					if(vnConfigListResult[i].dp[j].dpId == dpId){
						for(var k = 0; k < vnConfigListResult[i].dp[j].ports.length; k++){
							if(vnConfigListResult[i].dp[j].ports[k] == port){
								return true;//端口存在vn中
							}
						}
						return false;//节点存在但是端口不存在；
					}
				}
				return false;//节点端口不存在；
			}
		}
		return false;//节点对应配置不存在；
	}
