		...
		for(var i = 0; i < topoConfigListResult.length; i++){//遍历topo list记录信息；
			if(parseInt(topoConfigListResult[i].timeStamp / 1000) == time / 1000){
				...
				for(var j = 0; j < topoConfigListResult[i].node.length; j++){//拼凑一个node
					...
					inforToShow += "<td rowspan=" + lengthOfNode + ">第"+ (j+1) +"交换机</td><td>交换机id</td><td>"+ topoConfigListResult[i].node[j].nodeId +"</td></tr><tr><td>datapathId</td><td>"+topoConfigListResult[i].node[j].datapathId+"</td></tr></tr><tr><td>switchIp</td><td>"+topoConfigListResult[i].node[j].switchIp+"</td></tr></tr><tr><td>switchType</td><td>"+topoConfigListResult[i].node[j].switchType+"</td></tr></tr><tr><td>controllerIp</td><td>"+topoConfigListResult[i].node[j].controllerIp+"</td></tr><tr><td rowspan=";
					if(topoConfigListResult[i].node[j].terminationPoint.length == 0){
				...
				//拼凑link表格
				inforToShow += "<tr><td rowspan=" + topoConfigListResult[i].link.length +">链路信息</td><td colspan=3>"+topoConfigListResult[i].link[0].source.sourceTp+"<---->"+topoConfigListResult[i].link[0].destination.destTp+"</td></tr>"
