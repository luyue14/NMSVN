	...
	function findLink(dpId){
		for(var i = 0; i < topoResult.link.length; i++){
			if(topoResult.link[i].destination.destTp == dpId){
				return "<--->" + topoResult.link[i].source.sourceTp;
			}
			if(topoResult.link[i].source.sourceTp == dpId){
				return "<--->" + topoResult.link[i].destination.destTp;
			}
		}
		return "";
	}
				...
				for(var j = 0; j < topoResult.node[i].terminationPoint.length; j++){
					if(!isContains(topoResult.node[i].terminationPoint[j].tpId,"LOCAL")){
						if(isdpShouldCheck(topoResult.node[i].nodeId,topoResult.node[i].terminationPoint[j].tpId)){
							addNode.content.splice(6,0,"<input type='checkbox' name='nodeDp' checked='checked' class='list' value=" + topoResult.node[i].terminationPoint[j].tpId + ">" + topoResult.node[i].terminationPoint[j].tpId + "</input>" + findLink(topoResult.node[i].terminationPoint[j].tpId) + "</br>");
						}else{
							addNode.content.splice(6,0,"<input type='checkbox' name='nodeDp' class='list' value=" + topoResult.node[i].terminationPoint[j].tpId + ">" + topoResult.node[i].terminationPoint[j].tpId + "</input>" + findLink(topoResult.node[i].terminationPoint[j].tpId) + "</br>");
						}
					}
				}
				break;
	...
	function popVnInforToShow(time){//展示list中的详细信息
		...
		for(var i = 0; i < vnConfigListResult.length; i++){//遍历vn记录信息；
			if(parseInt(vnConfigListResult[i].timeStamp / 1000) == time / 1000){
				...
				var inforToShow = "<table class='tableList popFcolor'><tr><td>虚拟网Id</td><td colspan='3'>" 
				+vnConfigListResult[i].vnId + "</td></tr><tr><td>虚拟网名称</td><td colspan='3'>" 
				+ vnConfigListResult[i].vnName + "</td></tr><tr><td>虚拟网控制器Ip</td><td colspan='3'>" 
				+ vnConfigListResult[i].controllerIp + "</td></tr><tr><td>Ipv4源地址</td><td colspan='3'>" 
				+ vnConfigListResult[i].flowSpace.ipv4Src + "</td></tr><tr><td>Ipv4目的地址</td><td colspan='3'>" 
				+ vnConfigListResult[i].flowSpace.ipv4Dst + "</td></tr><tr><td rowspan='" 
				+ (vnConfigListResult[i].dp.length + 1) + "'>内部节点配置</td><th>交换机id</th><th>端口号</th><th>流表数量</th></tr>";
